...
nobleBindings.startScanning = function(serviceUuids, allowDuplicates,peripheralUuid) {
  this._scanServiceUuids = serviceUuids || [];
    this._scanPeripheralUuid = peripheralUuid;  // so sanh Peri. UUID
...
nobleBindings.onDiscover = function(address, addressType, advertisement, rssi) {
  ...
  var hasScanPeripheralUuid = (this._scanPeripheralUuid === undefined)
  ...
  if (!hasScanServiceUuids) {
    ...
    if(!hasScanPeripheralUuid)
    {
        var uuid = address.split(':').join('').toLowerCase();
        hasScanPeripheralUuid = (this._scanPeripheralUuid === uuid)
    }

  if (hasScanServiceUuids && hasScanPeripheralUuid) {
    var uuid = address.split(':').join('').toLowerCase();
      if(!hasScanPeripheralUuid)
